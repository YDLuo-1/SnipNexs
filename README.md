...
- The main window stays hidden during capture. Successfully copied, saved, or pinned captures are written to `history` beside `SnipNexs.exe`; press `,` for the previous image and `.` for the next one. The history keeps at most 20 original PNGs and about 64 MiB, and is restored after restart. If the application directory is not writable (for example, `Program Files`), it falls back to the Windows local application-data directory.
