...
- `app/AboutDialog` 独立显示版本、项目许可、Qt 许可与第三方声明；文本作为 Qt 资源内嵌，离线可读，不复制其他截图软件的界面或素材。主界面、关于页和许可页显式定义深色高对比度配色，链接不依赖系统默认的深蓝色。
...
- `capture/CaptureHistoryStore` 将成功复制、保存或贴出的原始截图以 PNG 写入 `SnipNexs.exe` 同目录的 `history` 文件夹，并以原子替换维护 `index.json`；`,` / `.` 向前或向后浏览，程序重启后仍可加载。若程序目录不可写，则回退到 Windows 本地应用数据目录。历史同时受 20 条和约 64 MiB 上限约束，损坏或缺失条目会在启动时静默跳过并清理。历史原图与缩小预览矩形分开保存，导出始终保留原始物理分辨率和图片自己的 DPR；历史模式固定整图选择，标注坐标在导出时映射回原图。
