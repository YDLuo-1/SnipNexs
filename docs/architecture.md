...
- `capture/CaptureOverlay` 负责窗口悬停预选、手动选区、提示与工具栏光标、标注交互和轻量绘制；原始 `QPixmap` 保持隐式共享，用户确认后才转换和裁剪为 `QImage`。`platform/windows/WindowTargeting` 只枚举可见、未最小化且未被 DWM 隐藏的顶层窗口，使用 DWM 可见边界并映射到当前显示器逻辑坐标，不引入控件级 UI Automation。
